...
def InitialLength(parameter):
    ...
    PE1_force_initial = c1 * k1 * np.log(np.exp((parameter.Lce_initial/parameter.L0/parameter.Lmax - Lr1)/float(k1))+1)*np.cos(parameter.alpha); #PE1 force at muscle length = Lce_initial defined by the user
    ...
    Lmt_initial = parameter.Lce_initial*np.cos(parameter.alpha)+Lse_initial*L0T
    Lce_initial = parameter.Lce_initial/float(L0)
...
def contraction_dynamics(U_eff,f_eff_slow,f_eff_fast,W_slow,W_fast,Y,S,Lse,x,parameter):
    ...
    FP1 = F_pe_1_function(x[0]/(parameter.L0/100)/float(parameter.Lmax),x[1]/(parameter.L0/100));
    FP2 = F_pe_2_function(x[0]/(parameter.L0/100));
    if FP2 > 0:
        FP2 = 0;     
    ...
    A_m = np.array([[0,1],[0,0]]);
    A_m[1,1] = temp
    B_m = np.array([[0],[float(parameter.mass)]])
